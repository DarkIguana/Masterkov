...
class specialists {
	...
	function __construct() {
		$over = "";
		$out = "";
		$id = isset ( $_GET ['specialists'] ) && is_numeric ( $_GET ['specialists'] ) ? ( int ) $_GET ['specialists'] : 0;
		
		$_SESSION ['Titles'] ['title'] .= ' / Специалисты';
		$_SESSION['Road'].="<a href='?specialists'>Специалисты</a>";
		if ($id != 0) {
			
			$sql = mysql_query ( "select * from `specialists` where `id`='{$id}' and `active`='1' limit 1" );
			if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
				$row = mysql_fetch_assoc ( $sql );
				//$out .= "<h2>{$row['name']}</h2>";
				$_SESSION['Road'].=" / <a href='?specialists={$row['id']}'>{$row['name']}</a>";
				$_SESSION ['Titles'] ['title'] .= ' / ' . $row ['name'];
				//$out .= '<div class="specialists" style="background:none;">';
				$time = strtotime ( $row ['date'] );
				$day = date ( "d", $time );
				$year = date ( "Y", $time );
					$img = $this->getItemImage($row['id']);
					if (!preg_match("/staff/is", $img['min'])){
						$img = '<img src="'.$img['big'].'" width="150" align="right"  style="padding:10px;"/>';
					}	
					else {
						$img = '';
					}	
				$out .= '  '.$img.'
				          <h3 style=" color: #000000;font: bold 14px Arial; padding:0px">'.$row['name'].'</h3>
                                <br/>
                              
                                ' . $row ['desc'] . '&nbsp;&nbsp;
                                
                          <p align="right"><a href="?specialists">Вернуться в раздел "Специалисты"</a></p>
                                ';
			...
			$sql = mysql_query ( "select * from `specialists` where `active`='1' order by `date` desc, `pos` asc limit $start, $limit" );
        ...
        '.$row['age'].' '.$nage.' стажа</p>
			   ...
			   $out .= '';
				...
				$count = mysql_result ( mysql_query ( "select count(*) from `specialists` where `active`='1'" ), 0 );
				...
				if ($totalPages > 1) {
					...
					for($i = 1; $i <= $totalPages; $i ++) {
						if ($i == $nowPage) {
							...
							$out .= '<a href="?specialists&page=' . $i . '">' . $i . '</a>&nbsp;&nbsp;';
...
function getItemImage($id) {
		...
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			...
			return array ("min" => "files/specialists/s_{$row['id']}.{$row['ext']}", "big" => "files/specialists/b_{$row['id']}.{$row['ext']}", "orig" => "o_{$row['id']}.{$row['ext']}" );
		}
		return array ("min" => "/template/default/images/staff.png", "big" => '/template/default/images/staff.png' );
	}
}
class specialists_admin extends glb {
	var $over = "";
	function __construct() {
	...
	function Update() {
		...
		mysql_query ( "update specialists set pos='$pos', active='$Active' where id='$_POST[id]'" );
		echo "33";
	}
	function UpdateImagePos() {
		$id = isset ( $_POST ['id'] ) ? ( int ) $_POST ['id'] : 0;
		$pos = isset ( $_POST ['pos'] ) ? ( int ) $_POST ['pos'] : 0;
		mysql_query ( "update `specialists_img` set `pos`='{$pos}' where `id`='{$id}' limit 1" );
		echo "33";
	}
	function UploadPhoto() {
		if (isset ( $_POST ['id'] ) && ! empty ( $_POST ['id'] )) {
			mysql_query ( "insert into `specialists_img` values ('', '$_POST[id]','', '', '0')" );
			$id = mysql_insert_id ();
			$uploaddir = $_SERVER ['DOCUMENT_ROOT'] . "/files/specialists/";
				...
				if (in_array ( $ext, array ("jpg", "jpeg", "png", "gif" ) )) {
					...
					if (move_uploaded_file ( $_FILES ['photo-path'] ['tmp_name'], $uploadfile3 )) {
						...
						if (copy ( $uploadfile1, $uploadfile2 )) {
							$this->ResizeImage ( $uploadfile2, $uploadfile2, 100, 100 );
						}
						
						mysql_query ( "update `specialists_img` set `ext`='$ext' where `id`='$id' limit 1" );
					} else {
						mysql_query ( "delete from `specialists_img` where `id`='$id' limit 1" );
						echo "{failure:true}";
						exit ();
					}
	...
	function getColumns() {
		$cols = array ();
		$sql = mysql_query ( "SHOW COLUMNS FROM `specialists`" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			while ( ($row = mysql_fetch_assoc ( $sql )) != false ) {
	...
	function save() {
		...
		mysql_query ( "update `specialists` set {$this->A2Up($fields, $values)} where `id`='$id'" ) or die ( "{failure:true, error:'" . addslashes ( mysql_error () ) . "'}" );
		echo "{success:true}";
	}
	function Listing() {
		if (! isset ( $_POST ['id'] )) {
		...
		$sql_count = "SELECT * FROM `specialists`";
		...
		if ($rows > 0) {
			while ( $obj = mysql_fetch_array ( $rs, MYSQL_ASSOC ) ) {
				$record = $obj;
				$record ['link'] = "?specialists=$obj[id]";
	...
	function deleteItem() {
		...
		$sql = mysql_query ( "select * from `specialists_img` where `iditem`='$_POST[id]'" );
		while ( $row = mysql_fetch_array ( $sql ) ) {
			$dir = $_SERVER ['DOCUMENT_ROOT'] . "/files/specialists/";
			...
			mysql_query ( "delete from `specialists_img` where `id`='$row[id]' limit 1" );
		}
		mysql_query ( "delete from `specialists` where `id`='$_POST[id]'" );
		echo "33";
	}
	function deleteImage() {
		$id = ( int ) $_POST ['id'];
		$sql = mysql_query ( "select * from `specialists_img` where `id`='$id' limit 1" );
	...
	function setOsnImage() {
		$id = ( int ) $_POST ['id'];
		$sql = mysql_query ( "select `id`,`iditem` from `specialists_img` where `id`='$id' limit 1" );
		$row = mysql_fetch_array ( $sql );
		mysql_query ( "update `specialists_img` set `osn`='0' where `iditem`='$row[iditem]'" );
		mysql_query ( "update `specialists_img` set `osn`='1' where `id`='$row[id]'" );
	}
	function Listing_Images() {
		$id = ( int ) $_POST ['dd'];
		$_POST ['start'] = isset ( $_POST ['start'] ) ? ( int ) $_POST ['start'] : 0;
		$_POST ['limit'] = isset ( $_POST ['limit'] ) ? ( int ) $_POST ['limit'] : 25;
		$sql_count = "SELECT * FROM `specialists_img` where `iditem`='$id'";
		$sql = $sql_count . " LIMIT " . ( int ) $_POST ['start'] . ", " . ( int ) $_POST ['limit'];
		$rs_count = mysql_query ( $sql_count ) or die ( "kjdfsk" );
		$rows = mysql_num_rows ( $rs_count );
		$rs = mysql_query ( $sql ) or die ( "HELLO" );
	...
	function NewItem() {
		mysql_query ( "insert into `specialists` (`id`, `active`) value ('', '0')" );
		$id = mysql_insert_id ();
		echo $id;
	}
