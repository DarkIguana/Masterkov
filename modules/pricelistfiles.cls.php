<?php 
class pricelistfiles_admin extends glb {
	...
	function Update() {
		...
		mysql_query ( "update pricelistfiles set pos='$pos', active='$Active', `catalog`='{$catalog}' where id='$_POST[id]'" );
		echo "33";
	}
	...
	function save() {
		...
		if ($id == 0 or empty ( $id )) {
			mysql_query ( "insert into `pricelistfiles` (`id`, `active`) values ('', '1')" );
			$id = mysql_insert_id ();
		}
		$fields = array ();
		$values = array ();
		...
		$fields ['date'] = 'date';
		$values ['date'] = time ();
		mysql_query ( "update `pricelistfiles` set {$this->A2Up($fields, $values)} where `id`='$id'" ) or die ( "{failure:true, error:'" . addslashes ( htmlspecialchars ( mysql_error () ) ) . "'}" );
		$this->uploadPhoto ( $id, true );
		echo "{success:true}";
	
		//echo "\nupdate `pricelistfiles` set {$this->A2Up($fields, $values)} where `id`='$id'";
	}
			...
			if (isset ( $_FILES ['photo-path'] ) && isset ( $_FILES ['photo-path'] ['name'] ) && ! empty ( $_FILES ['photo-path'] ['name'] )) {
				...
				if (isset ( $pathinfo ['extension'] )) {
					...
					if (! in_array ( $ext, $allow )) {
						...
						$newFile = 'files/pricelist/' . $fileName . '_' . $id . '.' . $ext;
						if ($deleteOld == true) {
							$sql = mysql_query ( "select  `ext`, `id`, `fileName` from `pricelistfiles` where `id`='$id'" );
							if (mysql_num_rows ( $sql ) > 0) {
								$row = mysql_fetch_assoc ( $sql );
								if (! empty ( $row ['ext'] ) && file_exists ( "files/pricelist/{$row['fileName']}_{$row['id']}.{$row['ext']}" )) {
									@unlink ( "files/pricelist/{$row['fileName']}_{$row['id']}.{$row['ext']}" );
								}
							}
						}
						if (move_uploaded_file ( $_FILES ['photo-path'] ['tmp_name'], $newFile )) {
							...
							mysql_query ( "update `pricelistfiles` set `ext`='$ext', `fileName`='$fileName' where `id`='$id'" );
						}
					}
				}
			}
	...
	function Listing() {
		...
		$sql_count = "SELECT * FROM `pricelistfiles` order by `pos` asc";
		...
		if ($rows > 0) {
			while ( $obj = mysql_fetch_array ( $rs, MYSQL_ASSOC ) ) {
				$record = $obj;
				if (file_exists ( "files/pricelist/{$obj['fileName']}_{$obj['id']}.{$obj['ext']}" )) {
					$record ['link'] = "files/pricelist/{$obj['fileName']}_{$obj['id']}.{$obj['ext']}";
					$record ['filesize'] = number_format ( filesize ( "files/pricelist/{$obj['fileName']}_{$obj['id']}.{$obj['ext']}" ) / 1024, 0, "", " " ) . " kb";
				} else {
	...
	function deleteItem() {
		...
		$sql = mysql_query ( "select  `ext`, `id` from `pricelistfiles` where `id`='$id'" );
		if (mysql_num_rows ( $sql ) > 0) {
			$row = mysql_fetch_assoc ( $sql );
			if (! empty ( $row ['ext'] ) && file_exists ( "files/pricelist/{$row['fileName']}_{$row['id']}.{$row['ext']}" )) {
				@unlink ( "files/pricelist/{$row['fileName']}_{$row['id']}.{$row['ext']}" );
		...
		mysql_query ( "delete from `pricelistfiles` where `id`='$_POST[id]'" );
		echo "33";
	}
