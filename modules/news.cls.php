...
class news {
	...
	function __construct() {
		...
		$id = isset ( $_GET ['news'] ) && is_numeric ( $_GET ['news'] ) ? ( int ) $_GET ['news'] : 0;
		
		$out .= '';
		if ($id != 0) {
			$out .= '<div class="news">';
			BreadcrumbsTitle::add ( 'Новости' );
			Breadcrumbs::add ( '<a href="?news">Новости</a>' );
			$sql = mysql_query ( "select * from `news` where `id`='{$id}' and `active`='1' limit 1" );
			if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
				...
				Breadcrumbs::add ( '<a href="?news=' . $row ['id'] . '">' . $row ['name'] . '</a>' );
				// $out .= '<div class="news" style="background:none;">';
...
<p>Цена: <span><b>'.$row2['price_rozn'].'</b></span></p>
</li>';
						}
						$out .= '</ul></div>';
					}
                ...
                //var_dump($img);
				$out .= '<tr>
						<td><span>' . date ( "d", $time ) . ' ' . $this->getMonth ( $time ) . ' ' . date ( 'Y', $time ) . '</span>
						<p>' . $row ['desc'] . '</p></td>
					</tr>';
				
				$out .= '</table><p class="news_back"><a href="/news">Вернуться в раздел "Новости"</a></p>';
				$out .= '</div>';
			} else {
				Response::error404 ();
			...
			BreadcrumbsTitle::set ( 'Новости компании Мастерков - разработка дизайна и ремонт квартир в Москве'.$dop );
			$_SESSION['Titles']['desc']='Новости компании Мастерков. Новые материалы и технологии на рынке строительства и ремонта. Советы от специалистов компании Мастерков.';
			$_SESSION['Titles']['keys']='новости компании Мастерков';
			Breadcrumbs::add ( '<a href="?news">Новости</a>' );
			...
			$sql = mysql_query ( "select * from `news` where `active`='1'  order by `date` desc, `pos` asc limit $start, $limit" );
			$out .= '<div class="news">';
			...
			if (is_resource ( $sql ) && ($num = mysql_num_rows ( $sql )) > 0) {
				...
				while ( ($row = mysql_fetch_assoc ( $sql )) != false ) {
					...
					$time = strtotime ( $row ['date'] );
					$img .= '&nbsp;';
					$out .= '<li><span>' . date ( "d.m.Y", $time ) . '&nbsp;&nbsp;</span><a href="/news/'.$link.'">' . $row ['name'] . '</a></li>';
				}
				
				$out .= '</ul>';
				
				$count = mysql_result ( mysql_query ( "select count(*) from `news` where `active`='1'" ), 0 );
...
HTML;
	...
	function getLastYear() {
		$sql = mysql_query ( "select YEAR(`date`) from `news` group by YEAR(`date`) order by YEAR(`date`) desc limit 1" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
	...
	function getYears() {
		$out = '';
		$sql = mysql_query ( "select YEAR(`date`) from `news` where `active`='1' group by YEAR(`date`) order by YEAR(`date`) desc limit 3" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			// $out .= '<ul>';
			$year = array ();
			while ( ($row = mysql_fetch_array ( $sql, MYSQL_NUM )) ) {
				if ($this->nowYear == $row [0]) {
					...
					$year [] = "<a href='?news&year={$row[0]}'>{$row[0]}</a>";
				}
			}
			$out .= implode ( "&nbsp;|&nbsp;", $year );
			// $out .= '</ul>';
...
class news_admin extends glb {
	...
	function Update() {
		...
		mysql_query ( "update news set pos='$pos', active='$Active' where id='$_POST[id]'" );
		echo "33";
	}
	function UpdateImagePos() {
		$id = isset ( $_POST ['id'] ) ? ( int ) $_POST ['id'] : 0;
		$pos = isset ( $_POST ['pos'] ) ? ( int ) $_POST ['pos'] : 0;
		mysql_query ( "update `news_img` set `pos`='{$pos}' where `id`='{$id}' limit 1" );
		echo "33";
	}
	function UploadPhoto() {
		if (isset ( $_POST ['id'] ) && ! empty ( $_POST ['id'] )) {
			mysql_query ( "insert into `news_img` values ('', '$_POST[id]','', '', '0')" );
			$id = mysql_insert_id ();
			$uploaddir = $_SERVER ['DOCUMENT_ROOT'] . "/files/news/";
			...
			if (isset ( $p ['extension'] )) {
				...
				if (in_array ( $ext, array (
					...
					if (move_uploaded_file ( $_FILES ['photo-path'] ['tmp_name'], $uploadfile3 )) {
						...
						mysql_query ( "update `news_img` set `ext`='$ext' where `id`='$id' limit 1" );
					} else {
						mysql_query ( "delete from `news_img` where `id`='$id' limit 1" );
						echo "{failure:true}";
						exit ();
					}
				} else {
	...
	function getColumns() {
		$cols = array ();
		$sql = mysql_query ( "SHOW COLUMNS FROM `news`" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			while ( ($row = mysql_fetch_assoc ( $sql )) != false ) {
	...
	function save() {
		...
		mysql_query ( "update `news` set {$this->A2Up($fields, $values)} where `id`='$id'" ) or die ( "{failure:true, error:'" . addslashes ( mysql_error () ) . "'}" );
		echo "{success:true}";
	}
	function Listing() {
		if (! isset ( $_POST ['id'] )) {
		...
		$sql_count = "SELECT * FROM `news` order by `date` desc";
	...
	function deleteItem() {
		$sql = mysql_query ( "select * from `news_img` where `iditem`='$_POST[id]'" );
		while ( $row = mysql_fetch_array ( $sql ) ) {
			$dir = $_SERVER ['DOCUMENT_ROOT'] . "/files/news/";
			...
			mysql_query ( "delete from `news_img` where `id`='$row[id]' limit 1" );
		}
	...
	function deleteImage() {
		$id = ( int ) $_POST ['id'];
		$sql = mysql_query ( "select * from `news_img` where `id`='$id' limit 1" );
	...
	function setOsnImage() {
		$id = ( int ) $_POST ['id'];
		$sql = mysql_query ( "select `id`,`iditem` from `news_img` where `id`='$id' limit 1" );
		$row = mysql_fetch_array ( $sql );
		mysql_query ( "update `news_img` set `osn`='0' where `iditem`='$row[iditem]'" );
		mysql_query ( "update `news_img` set `osn`='1' where `id`='$row[id]'" );
	}
	function Listing_Images() {
		$id = ( int ) $_POST ['dd'];
		$_POST ['start'] = isset ( $_POST ['start'] ) ? ( int ) $_POST ['start'] : 0;
		$_POST ['limit'] = isset ( $_POST ['limit'] ) ? ( int ) $_POST ['limit'] : 25;
		$sql_count = "SELECT * FROM `news_img` where `iditem`='$id'";
		$sql = $sql_count . " LIMIT " . ( int ) $_POST ['start'] . ", " . ( int ) $_POST ['limit'];
		$rs_count = mysql_query ( $sql_count ) or die ( "kjdfsk" );
		$rows = mysql_num_rows ( $rs_count );
		$rs = mysql_query ( $sql ) or die ( "HELLO" );
	...
	function NewItem() {
		mysql_query ( "insert into `news` (`id`, `active`) value ('', '0')" );
		$id = mysql_insert_id ();
		echo $id;
	}
