...
class map extends glb {
	...
	function getCatalogCats($parentId = 0) {
		$out = '';
		$sql = mysql_query ( "select `id`, `name` from `shop_cat` where `parentId`='{$parentId}' order by `sort`" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			$out .= "<ul style='list-style:none;margin-left:10px;'>";
			while ( ($row = mysql_fetch_assoc ( $sql ))!=false ) {
				$out .= "<li style='list-style:none;'><a href='?shop&catid={$row['id']}'>{$row['name']}</a>{$this->getCatalogCats($row['id'])}{$this->getItems($row['id'])}</li>";
			}
			$out .= "</ul>";
		}
		return $out;
	...
	function getItems($catId) {
		$out = '';
		$sql = mysql_query ( "select `id`, `name` from `shop_items` where `cat_id`='{$catId}' and `cat_id`!=0" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			$out .= "<ul style='list-style:none;margin-left:10px;'>";
			while ( ($row = mysql_fetch_object ( $sql ))!=false ) {
				
					$out .= "<li style='list-style:none;'><a href='?shop&catid={$row->id}'>{$row->name}</a></li>";
	...
	function getArticles() {
		$out = "<li style='list-style:none;'><a href='?special'><strong>Статьи</strong></a>";
		$sql = mysql_query ( "select `id`, `name` from `news` where `active`='1'" );
		if (is_resource ( $sql ) && mysql_num_rows ( $sql ) > 0) {
			$out .= "<ul style='padding-left:10px;list-style:none;'>";
			while ( ($row = mysql_fetch_assoc ( $sql ))!=false ) {
				$out .= "<li style='list-style:none;'><a href='?articles={$row['id']}'>{$row['name']}</a></li>";
			}
			$out .= "</ul>";
		}
		$out .= "</li>";
