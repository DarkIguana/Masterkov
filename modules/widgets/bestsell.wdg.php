...
class bestsell {
	...
	function __construct() {
		$out = '';
		$sql = mysql_query ( "select `shop_items`.`id`, `shop_items`.`name`, `shop_items`.`price_rozn`, `shop_items`.`photo`, `shop_cat`.`name` as `catname` from `shop_items`, `shop_cat` where `shop_cat`.`id`=`shop_items`.`cat_id` and `bestsell`='1' order by rand() limit 1" );
