...
class advice extends glb {
	...
	function __construct() {
		if (isset($_POST['ajax'])){
			...
			$sql = mysql_query("select * from `advice` where `active`!=2 and `id`!='{$id}' order by rand() desc limit 1");
			if (is_resource($sql) && mysql_num_rows($sql)>0){
				$row = mysql_fetch_assoc($sql);
				$out= '
			
					
                	<b>Советы от Мастеркова</b>
	...
	';
		...
		if (isset ( $_POST ['submitReview'] )) {
			...
			if (! $this->test ( $name ) or ! $this->test ( $email ) or ! $this->test ( $answer ) or ! $this->test ( $code )) {
				...
				$time = time ();
				mysql_query ( "insert into `advice` (`name`, `email`, `text`, `active`, `date`) value ('$name', '$email',  '$answer', '1', '$time')" );
				...
				$error .= "<p align='center' style='color:red'><b>Ваш совет успешно добавлен!</b></p>";
			}
		}
		
		$d = '';
		BreadcrumbsTitle::set ( "Советы от Мастеркова" );
		Breadcrumbs::add ( "<a href=\"?advice\">Советы</a>" );
		$page = isset ( $_GET ['page'] ) ? $_GET ['page'] - 1 : 0;
		$limit = 10;
		$start = abs ( $limit * $page );
		$sql = mysql_query ( "select * from `advice` where `active`!=2 order by `date` desc limit $start, $limit" );
		$count = mysql_result ( mysql_query ( "select COUNT(*) from `advice` where `active`!=2" ), 0 );
		
		if ($count == 0) {
			$this->over = "<p>Нет ни одного совета</p>" . $this->form ( $error );
			return true;
		}
	...
	function form($error) {
		 ...
		 * `advice` (`name`, `email`, `text`, `active`, `date`) value ('$name',
		 ...
		 */
		$d .= ' <p align="center">&nbsp;</p>
		<p class="p_advice">Оставить совет</p>
		<a name="addComment"></a>
...
<br/>
      <input type="submit" class="button56" name="submitReview" value="Оставить совет" /></form>
      </form></td>
        </tr>
      </table>
...
class advice_admin extends admin {
	function __construct() {
	...
	function SaveRecord() {
		...
		if (empty ( $id ) or $id == 0) {
			mysql_query ( "insert into `advice` (`name`, `text`, `active`, `email`, `answer`, `city`, `homepage`, `date`) value ('$name', '$text', '1', '$email', '$answer', '$city', '$homepage', '$date')" );
		} else {
			mysql_query ( "update `advice` set `name`='$name', `email`='$email', `city`='$city', `homepage`='$homepage', `text`='$text', `answer`='$answer', `date`='$date' where `id`='$id'" );
	...
	function UpdateRecord() {
		...
		if (! empty ( $id ) && $id != 0) {
			mysql_query ( "update `advice` set `active`='$active' where `id`='$id'" );
		}
		echo "33";
	}
	function DeleteRecord() {
		$id = isset ( $_POST ['id'] ) ? ( int ) $_POST ['id'] : 0;
		if (! empty ( $id ) && $id != 0) {
			mysql_query ( "delete from `advice` where `id`='$id' limit 1" );
		}
		echo "55";
	}
	function Listing() {
		$sql_count = "SELECT * FROM advice order by `active` desc";
