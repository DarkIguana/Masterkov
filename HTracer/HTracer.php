<?php
	//Version 3.3.1
	$GLOBALS['htracer_curent_version_id']=11;//Порядковый номер текущей версии. Нужен для синхронизации базы и версии
	...
	//Version 3.3.1 первая буржуйская
		//Увеличить словарь английских и русских слов за счет Гугл Translate
		//Парсинг гугла проводить не через API, это уберет часть глюков
		//Добавить возможность загрузки пользовательского словаря
		//Перенести в словарь остатки словарных констант из кода
		//С.4
	//Version 3.4 Для веб. студий
	
	 //	Основной упор, что через нее  Оптимизатору с кучей сайтов работать проще, чем через разные админки  
		//API для модулей
		//Мультиадминка
		//Модули
...
//*************Работа с ключевиком***************
	...
	{//Фильтр на запросы, возвращает код ошибки
		...
		if($Query=='not provided'||$Query=='not set')	
...
//ОБЛАКО	
	...
	static function FormCloud($Pages,$minsize,$maxsize,$mode,$shablon,$spacer,$HK=3)
	{
		...
		foreach ($Pages as $Page)
		{
			...
			$Replace= Array(
				'_HREF_'=>'http://'.$_SERVER['SERVER_NAME'].str_replace('&','&amp;',$Page['U']),
				'_SIZE_'=>$val,
				'_KEY_'=>$Page['Q'],
				'_KEY2_'=>$Page['Key2'],
				'_AMP_'=>'&'
	...
	//	error_reporting (E_ERROR | E_PARSE| E_WARNING);
		...
		$ArrGlobalNames=Array('htracer_cloud_links','htracer_cloud_randomize'
							 ...
							 ,'htracer_cloud_style','htracer_encoding','htracer_mysql_set_names');
		...
		if($Res)
		{
			...
			if($GLOBALS['htracer_test'])		
				$Res="  <!--noindex-->HT_CLOUD<!--/noindex--> ".$Res;	
			HTracer_Out();	
			return $Res;
		}		
		//Отдельный кеш для страниц облака (второй уровень) (уникален для каждой страницы, если испозован стиль вроде 5/15)
		$Pages=htracer_read_cash_file('CloudCashedPages',$CashCS,false,(($params['procent'] && $params['procent']!=100) || $params['urlstart']));
		if(!$Pages)
		{ 
			...
			$SelectMaxPagesCount=$params['count'];
			if(!$params['this'])
				$SelectMaxPagesCount++;
				...
				if(!$Pages)
				{
					...
					for($i=0;$i<$pc;$i++)
					{
						...
						if($params['upcase'])
						{
							...
							if ($GLOBALS['htracer_encoding']=='windows-1251' && $GLOBALS['htracer_mysql_set_names']=='utf8'){
								$cur['Key2']=iconv('utf-8', 'windows-1251', $cur['Key2']);
							}
	...
	static function ImportFromGA($Data,$echo=false)
	{
		...
		foreach($Data as $Str)
		{
			...
			if(count($arr)<5)
				continue;
			$URL=trim($arr[0]);//URL
			$Key=trim($arr[1]);//Запрос
			$Key=str_replace('*zpt*',',',$Key);
			$Key=trim($Key,'", ');//Запрос
			$Key=trim($Key);//Запрос
			$Num=trim($arr[3]);//Число
			if(strpos($Key,'*zpt*')||strpos($Num,'*zpt*'))
				continue;
			if(!$Key || stripos($Key,'no set')!==false|| stripos($Key,'not set')!==false)
				continue;
			if(!is_numeric($Num))
				continue;
			$Num=intval($Num);
...
//Вставка
	...
	{//Вставка альтов картинок, титлов ссылок и прочего		
		...
		if($AltCashCount<$AltCashPos)
			htracer_write_cash_file($AltCash,'InsertAltCash',$AltCashCS);
		if($TitleCashCount<count($TitleCash))	
		...
		if($GLOBALS['htracer_test'])
		{
			...
			$res=" <!--noindex--><br />HTracer:: Number of insertions/Avalible Places= $stat2/$stat 
	...
	{//Возвращает случайный ключевик
		...
		if($GLOBALS['htracer_mysql'])
		{
			...
			else
			{
				...
				if(HTracer::Rand('GetRandKey')%100<33 && $page['SecondKey'])
					$Out=$page['SecondKey'];
				else
					$Out=$page['FirstKey'];
			}
...
//Добавление в БД Ключей
	...
	{//Добавление текущего ключа (ключа по которому перешел пользователь на страницу)
		...
		// Маркеры гугл аналитикс
		if(isset($_GET['utm_source']) && $_GET['utm_source'])
		{
			HTracer_Out();
			return;
		}
		// Маркер Яндекс-директа
